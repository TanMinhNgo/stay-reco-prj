...
import { Building2, CalendarDays, LayoutDashboard, LogOut, MessageSquare, Settings } from "lucide-react";
...
export const defaultSidebarItems: SidebarItem[] = [
  { href: "/dashboard", label: "Tổng quan", icon: LayoutDashboard, isActive: true },
    ...
    >
      <SidebarHeader className="gap-0 border-b border-sidebar-border p-0">
        <div className="flex h-20 items-center px-6">
          <Link href="/" className="rounded-md focus-visible:outline-none" aria-label="StayReco - Trang chủ">
            <Image
      ...
      <SidebarContent>
        <SidebarGroup className="px-3 py-4">
          <SidebarGroupLabel className="px-3 text-xs text-muted-foreground">Điều hướng</SidebarGroupLabel>
          <SidebarGroupContent>
              ...
              {items.map((item) => {
                ...
                return (
                  <SidebarMenuItem key={`${item.href}-${item.label}`}>
                    <SidebarMenuButton
                      render={<Link href={item.href} aria-current={item.isActive ? "page" : undefined} />}
                    ...
                    {item.badge !== undefined && <SidebarMenuBadge className="bg-white font-semibold tabular-nums text-primary ring-1 ring-primary/15">{item.badge}</SidebarMenuBadge>}
                  </SidebarMenuItem>
                );
              })}
            </SidebarMenu>
