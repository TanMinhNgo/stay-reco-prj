...
import { BedDouble, CalendarDays, CreditCard, LayoutDashboard, MessageSquare, Settings } from "lucide-react";
...
const sidebarItems: SidebarItem[] = [
  { href: "#workspace", label: "Tổng quan", icon: LayoutDashboard, isActive: true },
      ...
      <SidebarInset className="min-w-0 bg-background">
        ...
        <main id="workspace" className="flex-1">
          <div className="mx-auto max-w-[100rem] space-y-8 px-4 py-7 sm:px-6 lg:px-8">
            <section>
              <p className="text-sm font-medium text-primary">StayReco UI foundation</p>
              <h1 className="sr-page-title mt-2">Tổng hợp giao diện dùng chung</h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                Trang thử nghiệm các component public và workspace theo hệ Coastal Blue & Warm Paper.
              </p>
            </section>
            ...
            <section aria-labelledby="public-header-title">
              <div className="mb-4 flex items-end justify-between gap-4">
                <div>
                  <h2 id="public-header-title" className="sr-section-title">Header khách hàng</h2>
                  <p className="mt-1 text-sm text-muted-foreground">Trạng thái đã đăng nhập, kèm menu ngôn ngữ và tiền tệ.</p>
                </div>
              </div>
            ...
            <section id="table" aria-labelledby="table-title" className="space-y-4">
              <div>
                <h2 id="table-title" className="sr-section-title">Danh sách đặt phòng</h2>
                <p className="mt-1 text-sm text-muted-foreground">Filter, table và pagination trong một luồng quản lý thực tế.</p>
              </div>
                  ...
                  <TableBody>
                    {bookings.map((booking) => (
                      <TableRow key={booking.code} className="h-14">
                        <TableCell className="px-4 font-semibold text-primary">{booking.code}</TableCell>
                        <TableCell className="px-4 font-medium">{booking.guest}</TableCell>
                        <TableCell className="px-4 text-muted-foreground">{booking.room}</TableCell>
                        <TableCell className="px-4 tabular-nums text-muted-foreground">{booking.dates}</TableCell>
                        <TableCell className="px-4 font-semibold tabular-nums">{booking.total}</TableCell>
                        <TableCell className="px-4">
                          <Badge className={booking.statusClass}>{booking.status}</Badge>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
            ...
            <section aria-labelledby="footer-title">
              <h2 id="footer-title" className="sr-section-title mb-4">Footer khách hàng</h2>
              <div className="overflow-hidden rounded-2xl border border-border shadow-card">
                <Footer />
              </div>
            </section>
