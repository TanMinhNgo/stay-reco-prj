      ...
      <div className="mx-auto max-w-[100rem] px-4 py-10 sm:px-8 lg:px-8 lg:py-11">
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-[1.75fr_0.8fr_1fr_0.8fr] lg:gap-12">
          <div className="max-w-[25rem]">
            <Link href="/" className="inline-block rounded-md focus-visible:outline-none" aria-label="StayReco - Trang chủ">
              <Image
            ...
            <p className="mt-4 text-sm leading-6 text-muted-foreground">
              Nền tảng đặt phòng khách sạn và boutique stay hàng đầu Việt Nam. Tận hưởng kỳ nghỉ với trải nghiệm chân thực,
              minh bạch giá cả và dịch vụ chăm sóc tận tâm.
            </p>
          </div>

          {footerGroups.map((group) => (
            <section key={group.title} aria-labelledby={group.title.replaceAll(" ", "-").toLowerCase()}>
              <h2 id={group.title.replaceAll(" ", "-").toLowerCase()} className="text-base font-semibold text-foreground">
                {group.title}
              </h2>
        ...
        <div className="mt-10 flex flex-col gap-4 border-t border-border pt-5 text-sm text-muted-foreground lg:mt-11 lg:flex-row lg:items-center lg:justify-between">
          <p className="leading-6">
            © {currentYear} StayReco Hospitality Platform. Bản quyền thuộc về StayReco. Nền tảng đặt phòng khách sạn boutique Việt Nam.
          </p>
          <div className="flex shrink-0 items-center gap-4" aria-label="Ngôn ngữ và tiền tệ">
            <button type="button" className="rounded-sm transition-colors hover:text-primary focus-visible:outline-none">
              Tiếng Việt (VN)
            </button>
